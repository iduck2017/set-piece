...
> { 
    ...
    public any<T extends Model>(type: IClass<T>): T['proxy'] {
        return new ProxyPlugin(this.model, [...this.keys, type]);
    }
    
    private getEvent(
        origin: Record<string, Producer>, 
        key: string
    ): Producer {
        if (!origin[key]) {
            origin[key] = { 
     ...
     private getChild(
        origin: Record<string, ProxyPlugin>, 
        path: string
    ): ProxyPlugin | undefined {
        const keys = path.split('/');
        const key = keys.shift();
        if (!key) return this;
        if (!origin[key]) {
            origin[key] = new ProxyPlugin(this.model, [...this.keys, key]);
        }
        if (keys.length) {
            const child: Record<string, ProxyPlugin> = origin[key].child;
