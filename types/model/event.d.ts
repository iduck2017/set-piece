...
export type EventProducers<E extends Record<string, any>, M extends Model> = {
    [K in keyof E]: EventProducer<Required<E>[K], M>;
};
export type OnStateChange<M extends Model> = {
    prev: Model.State<M>;
    next: Model.State<M>;
};
export type OnChildChange<M extends Model> = {
    prev: Model.Child<M>;
    next: Model.Child<M>;
};
export type OnReferChange<M extends Model> = {
    prev: Model.Refer<M>;
    next: Model.Refer<M>;
};
